...
// Détecte si la page est ouverte dans le navigateur intégré d'Instagram
// (ou Facebook, qui utilise le même moteur). Permet de bloquer l'accès
// hors de l'app pour limiter le partage/copie du lien.
...
export default function Wheel() {
  ...
  const [needsInstagram] = useState(() => !isInstagramBrowser());
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center" style={bgStyle}>
        <div className="bg-slate-900/70 rounded-2xl px-6 py-8 max-w-sm mt-32">
          ...
          <p className="text-xl text-white font-bold mb-2">
            Ouvre ce lien depuis Instagram
          </p>
          <p className="text-slate-300 text-sm">
            Pour jouer, clique sur le lien directement dans notre bio{" "}
            <a href={IG_LINK} className="text-amber-300 underline font-semibold">
              {IG_HANDLE}
            </a>{" "}
          ...
          <div className="bg-slate-800 rounded-2xl p-6 max-w-sm w-full text-center border border-slate-700">
            {won ? (
              <>
                ...
                <p className="text-white font-bold text-lg mb-2">@{pseudo.trim().replace(/^@+/, "")}</p>
                {result.image_slug && (
                  <img
                ...
                <div className="bg-amber-400/10 border border-amber-400/40 rounded-xl p-4">
                  ...
                  <p className="text-slate-300 text-sm mt-1">
                    et montrez-la nous à la caisse pour recevoir votre gain !
                  </p>
                </div>
              </>
            ) : (
